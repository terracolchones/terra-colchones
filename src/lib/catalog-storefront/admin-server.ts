...
const STORAGE_PATH_PATTERN =
  /^products\/[0-9a-f-]{36}\/[a-zA-Z0-9_-]{1,128}\.(png|jpe?g|webp)$/;
...
function requiredAdminPassword(): string {
  ...
  if (!password)
    throw new CatalogRequestError(
      "Falta configurar CATALOG_ADMIN_PASSWORD",
      503,
    );
  return password;
}
...
export async function requireCatalogAdmin(request: NextRequest): Promise<void> {
  if (!isCatalogConfigured())
    throw new CatalogRequestError("El catálogo no está configurado", 503);
...
function asRecord(value: unknown): Record<string, unknown> {
  if (!value || typeof value !== "object" || Array.isArray(value))
    throw new CatalogRequestError("El formato enviado no es válido");
...
function optionalText(value: unknown, maxLength: number): string | null {
  ...
  if (typeof value !== "string")
    throw new CatalogRequestError(
      "Un campo de texto tiene un formato inválido",
    );
  const normalized = value.trim();
  if (normalized.length > maxLength)
    throw new CatalogRequestError(
      `Un texto no puede superar ${maxLength} caracteres`,
    );
...
function productSlug(value: unknown, productName: string): string {
  ...
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(candidate)) {
    throw new CatalogRequestError(
      "La URL del producto solo admite letras minúsculas, números y guiones",
    );
  }
  return candidate;
}
...
function nonNegativeInteger(value: unknown, fallback: number): number {
  ...
  if (!Number.isInteger(normalized) || normalized < 0 || normalized > 9_999)
    throw new CatalogRequestError("El orden no es válido");
  return normalized;
}
...
export function parseProductInput(payload: unknown): ProductInput {
  ...
  const variants = rawVariants.map((item, index) => {
    ...
    if (id && !UUID_PATTERN.test(id))
      throw new CatalogRequestError(
        "El identificador de una variante no es válido",
      );
    ...
    const colorValue = showColor ? optionalText(variant.colorHex, 7) : null;
    const colorHex = normalizeColorHex(colorValue);
    if (colorValue && !colorHex)
      throw new CatalogRequestError("El color de la variante no es válido");
    if (showColor && !colorHex)
      throw new CatalogRequestError("Elige un color para la variante activada");
    const variantName = requiredText(
      variant.name ?? variant.label,
      "El nombre de la variante",
      140,
    );
    const label = showOptionText
      ? requiredText(variant.label, "El texto de la variante", 100)
      : (optionalText(variant.label, 100) ?? variantName);
    const images = rawVariantImages.map((item, imageIndex) => {
      const image = asRecord(item);
      const imageId = optionalText(image.id, 36);
      const path = requiredText(
        image.path,
        "La ruta de la foto de la variante",
        255,
      );
      if (imageId && !UUID_PATTERN.test(imageId))
        throw new CatalogRequestError(
          "El identificador de una foto de variante no es válido",
        );
      if (!VARIANT_STORAGE_PATH_PATTERN.test(path))
        throw new CatalogRequestError(
          "La foto de variante debe provenir del panel de catálogo",
        );
      return {
        id: imageId,
        path,
        alt:
          optionalText(image.alt, 180) ??
          `${name} - variante ${index + 1} - imagen ${imageIndex + 1}`,
        sortOrder: nonNegativeInteger(image.sortOrder, imageIndex + 1),
      };
    });
    return {
      id,
      ...
      name: variantName,
      slug: productSlug(variant.slug, `${slug}-${variantName}`),
      label,
      colorHex,
      colorName: showColor ? optionalText(variant.colorName, 60) : null,
      showColor,
      showOptionText,
  ...
  const images = rawImages.map((item, index) => {
    ...
    if (id && !UUID_PATTERN.test(id))
      throw new CatalogRequestError(
        "El identificador de una imagen no es válido",
      );
    if (!STORAGE_PATH_PATTERN.test(path))
      throw new CatalogRequestError(
        "La imagen debe provenir del panel de catálogo",
      );
    return {
      id,
      path,
...
function primaryVariantFromProduct(input: ProductInput): ProductVariantInput {
  return {
    id: null,
    ...
    compareAtPrice: input.compareAtPriceFrom,
...
/** Toda familia conserva una primera versión pública, incluso antes de crear las demás. */
function withPrimaryVariant(input: ProductInput): ProductInput {
  const variants =
    input.variants.length > 0
      ? [...input.variants]
      : [primaryVariantFromProduct(input)];
  const preferredPrimary = variants.findIndex((variant) => variant.isPrimary);
  const primaryIndex = preferredPrimary >= 0 ? preferredPrimary : 0;
  return {
    ...input,
...
async function replaceRelations(
  ...
  if (existingVariants.error) {
    if (existingVariants.error.message.includes("is_primary")) {
      throw new CatalogRequestError(
        "Falta activar la migración de versiones públicas, enlaces y galerías",
        503,
      );
    }
    throw new Error(existingVariants.error.message);
  }
  ...
  const existingPrimaryId = (existingVariants.data ?? []).find(
    (item) => item.is_primary === true,
  )?.id;
  if (
    existingPrimaryId &&
    !input.variants.some((variant) => variant.id === existingPrimaryId)
  ) {
    throw new CatalogRequestError("La versión principal no se puede eliminar.");
  }
  ...
  const variants = await Promise.all(
    input.variants.map(async (variant) => ({
      ...
      name: variant.name,
      slug: await uniqueVariantSlug(variant.slug, variant.id),
      label: variant.label,
      color_hex: variant.colorHex,
      color_name: variant.colorName,
      show_color: variant.showColor,
      show_option_text: variant.showOptionText,
  ...
  const removedVariantIds = (existingVariants.data ?? [])
  ...
  const variantImages = input.variants.flatMap((variant, index) =>
  ...
  const existingVariantIds = (existingVariants.data ?? []).map((item) =>
    String(item.id),
  );
  if (existingVariantIds.length > 0 || variantImages.length > 0) {
    ...
    if (existingVariantImages.error) {
      if (
        !catalogSchemaMissing(existingVariantImages.error) ||
        variantImages.length > 0
      )
        throw new Error(existingVariantImages.error.message);
    } else {
      if (variantImages.length > 0) {
        const { error } = await client
          .from("catalog_variant_images")
          .upsert(variantImages);
        if (error) throw new Error(error.message);
      }
      const keptVariantImageIds = new Set(
        variantImages.map((image) => image.id),
      );
      const removedVariantImageIds = (existingVariantImages.data ?? [])
        .map((item) => String(item.id))
        .filter((id) => !keptVariantImageIds.has(id));
      if (removedVariantImageIds.length > 0) {
        const { error } = await client
          .from("catalog_variant_images")
          .delete()
          .in("id", removedVariantImageIds);
        if (error) throw new Error(error.message);
      }
    }
  }
  ...
  const removedImageIds = (existingImages.data ?? [])
    .map((item) => String(item.id))
...
async function uniqueProductSlug(
  ...
  throw new CatalogRequestError(
    "No se pudo generar una URL única para el producto",
  );
}

async function uniqueVariantSlug(
  baseSlug: string,
  ...
  for (let suffix = 1; suffix <= 99; suffix += 1) {
    ...
    if (!data || String(data.id) === currentVariantId) return candidate;
  }
  throw new CatalogRequestError(
    "No se pudo generar una URL única para la variante",
  );
}
...
export async function createCatalogProduct(
  ...
  const completeInput = withPrimaryVariant({ ...input, slug });
  const { data, error } = await getCatalogServerClient()
    .from("catalog_products")
    .insert({
      external_code: completeInput.externalCode,
      slug,
      ...
      specifications: completeInput.specifications,
      price_from: completeInput.priceFrom,
      compare_at_price_from: completeInput.compareAtPriceFrom,
      availability: completeInput.availability,
      published: completeInput.published,
      featured: completeInput.featured,
      sort_order: completeInput.sortOrder,
    })
    .select("id")
    .single();
  if (error || !data?.id)
    throw new Error(error?.message || "No se pudo crear el producto");
  await replaceRelations(String(data.id), completeInput);
...
export async function updateCatalogProduct(
  ...
  if (!UUID_PATTERN.test(productId))
    throw new CatalogRequestError("El producto no es válido", 404);
  const slug = await uniqueProductSlug(input.slug, productId);
  const completeInput = withPrimaryVariant({ ...input, slug });
  const { error } = await getCatalogServerClient()
    .from("catalog_products")
    .update({
      external_code: completeInput.externalCode,
      slug,
    ...
    .eq("id", productId);
  if (error) throw new Error(error.message);
  await replaceRelations(productId, completeInput);
}
...
function imageMimeType(file: File): { mimeType: string; extension: string } {
  if (file.type === "image/png")
    return { mimeType: file.type, extension: "png" };
  if (file.type === "image/jpeg")
    return { mimeType: file.type, extension: "jpg" };
  if (file.type === "image/webp")
    return { mimeType: file.type, extension: "webp" };
...
export async function uploadCatalogImage(
  ...
  if (!UUID_PATTERN.test(productId))
    throw new CatalogRequestError("Guarda el producto antes de subir imágenes");
  if (file.size === 0 || file.size > MAX_IMAGE_BYTES)
    throw new CatalogRequestError("La imagen debe pesar menos de 5 MB");
  const { count, error: countError } = await getCatalogServerClient()
    .from("catalog_product_images")
  ...
  if (error) throw new Error(error.message);
  const { data } = getCatalogServerClient()
    .storage.from(CATALOG_BUCKET)
    .getPublicUrl(path);
  return { path, url: data.publicUrl };
}

export async function uploadCatalogVariantImage(
  productId: string,
  variantId: string,
  file: File,
): Promise<{ path: string; url: string }> {
  if (!UUID_PATTERN.test(productId) || !UUID_PATTERN.test(variantId))
    throw new CatalogRequestError("Guarda la variante antes de subir fotos");
  if (file.size === 0 || file.size > MAX_IMAGE_BYTES)
    throw new CatalogRequestError("La imagen debe pesar menos de 5 MB");
  const { data: variant, error: variantError } = await getCatalogServerClient()
    .from("catalog_variants")
    .select("id")
    .eq("id", variantId)
    .eq("product_id", productId)
  ...
  const { error: schemaError } = await getCatalogServerClient()
    .from("catalog_variant_images")
    .select("id")
  ...
  if (schemaError)
    throw new CatalogRequestError(
      "Falta activar la migración de galerías de variantes",
      503,
    );
  const { count, error: countError } = await getCatalogServerClient()
    .from("catalog_variant_images")
...
export async function deleteCatalogProduct(productId: string): Promise<void> {
  ...
  if (productImagesError || variantsError)
    throw new Error(
  ...
  if (error) throw new Error(error.message);
  const paths = [
    ...(productImages ?? []).map((image) => String(image.storage_path)),
    ...variantPaths,
  ].filter(Boolean);
  if (paths.length > 0) {
    ...
    if (storageError)
      console.error(
        "[catalog-admin] no se pudieron limpiar algunas fotos eliminadas:",
...
export async function getAdminHomeSettings(): Promise<CatalogHomeSettings> {
  ...
  const { data: publicUrl } = getCatalogServerClient()
    .storage.from(CATALOG_BUCKET)
  ...
  return {
    ...
    title:
      typeof data.title === "string" ? data.title : DEFAULT_CATALOG_HOME.title,
    description:
...
export async function saveAdminHomeSettings(payload: unknown): Promise<void> {
  ...
  if (imagePath && !STORAGE_PATH_PATTERN.test(imagePath))
    throw new CatalogRequestError(
      "La imagen de portada debe provenir del panel",
    );
  const { error } = await getCatalogServerClient()
    .from("catalog_home_settings")
    .upsert({
...
export function jsonError(error: unknown) {
  ...
  if (message.includes("catalog_products_slug_key")) {
    return NextResponse.json(
      { error: "Ya existe un producto con esa URL. Usa otro nombre o URL." },
      { status: 409 },
    );
  }
