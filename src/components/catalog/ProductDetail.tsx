...
function preferredVariant(
  ...
  current: CatalogVariant | null,
): CatalogVariant | null {
  return (
    candidates.find((variant) => variant.id === current?.id) ??
    candidates.find((variant) => variant.isPrimary) ??
    candidates[0] ??
    null
  );
}
...
export function ProductDetail({
  ...
  const selectedVariant = initialVariant(activeVariants, selectedVariantId);
  const textOptions = useMemo(
    () =>
      Array.from(
        new Set(
          activeVariants
            .filter((variant) => variant.showOptionText && variant.label.trim())
  ...
  const colorVariants = useMemo(() => {
    ...
    return activeVariants.filter((variant) => {
      if (!variant.showColor || !variant.colorHex || seen.has(variant.colorHex))
        return false;
  ...
  const selectionText = selectedVariant?.showOptionText
    ? selectedVariant.label
    : null;
  const selectionColor = selectedVariant?.showColor
  ...
  function selectVariant(next: CatalogVariant | null): void {
    if (!next) return;
    ...
    const query = checkoutToken
      ...
      : "";
    router.replace(
      `/catalogo/productos/${encodeURIComponent(next.slug || product.slug)}${query}`,
      { scroll: false },
    );
  }

  function selectTextOption(label: string): void {
    const candidates = activeVariants.filter(
      (variant) => variant.showOptionText && variant.label === label,
    );
    const matchingColor = candidates.find(
      (variant) =>
        selectedVariant?.showColor &&
        variant.colorHex === selectedVariant.colorHex,
    );
    selectVariant(
      matchingColor ?? preferredVariant(candidates, selectedVariant),
    );
  }

  function selectColorOption(colorHex: string): void {
    const candidates = activeVariants.filter(
      (variant) => variant.showColor && variant.colorHex === colorHex,
    );
    const matchingText = candidates.find(
      (variant) =>
        selectedVariant?.showOptionText &&
        variant.label === selectedVariant.label,
    );
    selectVariant(
      matchingText ?? preferredVariant(candidates, selectedVariant),
    );
  }
  ...
  async function confirmOrder(): Promise<void> {
    ...
    try {
      ...
      const whatsAppUrl = buildOrderConfirmationWhatsAppUrl(
        data.orderCode,
        whatsAppPhone,
      );
      if (!whatsAppUrl)
    ...
    if (!whatsAppPhone) {
      return (
        <button
          ...
          className={`inline-flex min-h-11 shrink-0 items-center justify-center rounded-xl bg-stone-300 px-4 text-sm font-bold text-stone-600 sm:min-h-12 sm:px-5 ${widthClass}`}
        >
    ...
    <main className="min-h-dvh bg-white pb-24 text-stone-900 lg:pb-14">
      <div className="mx-auto max-w-3xl lg:max-w-[76rem] lg:px-6 lg:py-10">
        ...
        <div className="lg:mt-5 lg:grid lg:grid-cols-[minmax(0,1fr)_minmax(0,0.94fr)] lg:items-start lg:gap-12 xl:gap-16">
                ...
                {(galleryImages.length > 0 ? galleryImages : [null]).map(
                  (image) => (
                    <div
                      ...
                      className="h-full w-full shrink-0 snap-center sm:aspect-square"
                    >
                      <ProductVisual
                        product={product}
                        imageUrl={image?.url ?? null}
                        alt={image?.alt ?? selectedName}
                      />
                    </div>
                  ),
                )}
              </div>
              <div className="hidden h-full sm:aspect-square lg:block">
                <ProductVisual
                  product={product}
          ...
          <section className="px-5 pb-1 pt-3 sm:pt-7 lg:px-0 lg:pt-7">
            <p className="text-[10px] font-bold tracking-[0.15em] text-[#9a2022] sm:text-[11px]">
              {product.category.toUpperCase()}
            </p>
            <h1 className="mt-1.5 text-[1.75rem] font-semibold leading-tight tracking-tight text-stone-950 sm:mt-2 sm:text-3xl lg:text-[2.65rem]">
              {selectedName}
            </h1>
            <div className="mt-2 flex items-end gap-2 sm:mt-4">
              <p className="text-xl font-extrabold tracking-tight text-stone-950 sm:text-2xl lg:text-3xl">
                {formatBolivianos(price)}
              </p>
              {compareAtPrice !== null && compareAtPrice > (price ?? 0) && (
                <p className="pb-1 text-sm text-stone-400 line-through">
                  {formatBolivianos(compareAtPrice)}
                </p>
              )}
            </div>
            {selectedVariant?.externalCode && (
              <p className="mt-2 text-xs font-medium text-stone-500">
                Código: {selectedVariant.externalCode}
              </p>
            )}
            <div className="mt-4 hidden items-center gap-2 text-sm font-medium text-stone-600 lg:inline-flex">
              <span
                className={`size-2 rounded-full ${currentAvailability === "available" ? "bg-emerald-500" : currentAvailability === "out_of_stock" ? "bg-stone-400" : "bg-amber-400"}`}
                aria-hidden="true"
              />
            ...
            {textOptions.length > 0 && (
              <section
                ...
                aria-labelledby="text-option-heading"
              >
                <div className="flex items-baseline justify-between gap-4">
                  <h2
                    id="text-option-heading"
                    className="text-sm font-bold text-stone-950"
                  >
                    Elige tamaño, modelo u opción
                  </h2>
                  {selectionText && (
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {textOptions.map((label) => (
                    <button
                      key={label}
                      type="button"
                      aria-pressed={selectionText === label}
                      onClick={() => selectTextOption(label)}
                      className={`inline-flex min-h-9 items-center rounded-full border px-3 text-sm font-semibold transition focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#8f1519] ${selectionText === label ? "border-stone-950 bg-stone-950 text-white" : "border-stone-200 bg-white text-stone-800 hover:border-stone-400"}`}
                    >
                      {label}
                    </button>
                  ))}
              ...
              >
                ...
                >
                  {colorVariants.map((variant, index) => {
                    ...
                    const colorName = variant.colorName || `Color ${index + 1}`;
                    return (
                      <button
                        ...
                        onClick={() => selectColorOption(variant.colorHex!)}
                      ...
                      >
                        <span
                          className="size-7 rounded-full border border-stone-300/60"
                          style={{
        ...
        <article className="mx-5 mt-8 border-t border-stone-200 pb-6 pt-6 sm:mx-0 sm:mt-7 sm:px-5 lg:mx-0 lg:mt-14 lg:px-0 lg:pb-0 lg:pt-9">
          ...
          {product.shortDescription && (
            <p className="mt-4 text-[15px] leading-6 text-stone-600 sm:mt-5 lg:mx-auto lg:max-w-4xl lg:text-center">
              {product.shortDescription}
            </p>
          )}
          {product.description && (
            <section className="mt-7 border-t border-stone-200 pt-6 lg:mx-auto lg:mt-10 lg:max-w-4xl lg:pt-8">
              <h2 className="text-sm font-bold text-stone-950 lg:text-center">
                Descripción
              </h2>
              <p className="mt-3 text-[15px] leading-6 text-stone-600 lg:text-center">
                {product.description}
              </p>
            </section>
          )}
      ...
      <div className="fixed inset-x-0 bottom-0 z-30 border-t border-stone-200 bg-white/95 px-4 py-2 backdrop-blur sm:py-3 lg:hidden">
        ...
        {orderError && (
          <p
            role="alert"
            className="mx-auto mt-1 max-w-3xl text-xs font-medium text-red-700"
          >
            {orderError}
          </p>
        )}
