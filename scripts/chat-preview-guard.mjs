...
if (process.env.TERRA_CHAT_PREVIEW !== "1" || !process.env.TERRA_CHAT_PREVIEW_ROOT) {
  throw new Error("Use npm run preview:chat to enable this guard.");
}

const root = path.resolve(process.env.TERRA_CHAT_PREVIEW_ROOT);
...
function permittedRoute(url) {
  return url.pathname === "/preview/chat" || url.pathname === "/preview/chat/" ||
...
for (const Server of [http.Server, https.Server]) {
  ...
  Server.prototype.emit = function (event, ...args) {
    if (event === "request" || event === "upgrade") {
      let url;
      try { url = new URL(args[0].url, "http://127.0.0.1:3191"); } catch { url = null; }
      if (!url || !permittedRoute(url)) {
        if (event === "upgrade") {
          ...
          if (url?.pathname === "/") {
            response.writeHead(307, { Location: "/preview/chat" });
            response.end();
          } else {
            response.writeHead(403, { "Content-Type": "text/plain; charset=utf-8" });
            response.end("Vista local: solo está habilitado el chat de demostración.");
          }
        }
        return true;
      }
...
// This preview has no database; synthetic data stays in browser memory.
